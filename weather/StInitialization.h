...
private:
    ...
    void generate_button_press(const std::string& name, const int duration){
        ...
        if( name == "btn_1"){
            if(duration <= 1){
                ...
                STM_ADD_EVENT(std::make_shared<smachine::Event>(smachine::EVENT_TYPE::EVT_USER, EVT_SHOW_IP));
            }
            ...
            if(duration >= 5){
                STM_ADD_EVENT(std::make_shared<smachine::Event>(smachine::EVENT_TYPE::EVT_FINISH));
            }
        }
    }
    ...
    void detect_ip_address(){
        ...
        std::string ip4_address = netInfo.detect_ip_address_by_type(piutils::netinfo::IpType::IP_V4);
        std::string ip6_address = netInfo.detect_ip_address_by_type(piutils::netinfo::IpType::IP_V6);
