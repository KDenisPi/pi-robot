...
#define DMA_REG_TI_DEST_IGNORE      (1<<11)                   //Ignore reads
#define DMA_REG_TI_DEST_DREQ        (1<<10)                   //Control Destination Reads with DREQ (1 - The DREQ selected by PERMAP will gate the destination read)
#define DMA_REQ_TI_DEST_WIDTH_128   (1<<9)                    //Destination transfer width (1-128bit, 0-32bit)
#define DMA_REQ_TI_DEST_INC         (1<<8)                    //Destination address increment (1-Yes, 0-No) Increment value is on depend of DEST_WIDTH
...
public:
    DmaControlBlock(const DREQ dreq) : _dreq(dreq), _ti_flags(0) {
