...
std::shared_ptr<provider::Provider> PiRobot::get_provider(const std::string& name) const{
    ...
    if(provider == providers.end()){
        logger::log(logger::LLOG::ERROR, __func__, " Absent requested Provider with ID " + name);
...
void PiRobot::add_provider(const std::string& type, const std::string& name){
    ...
    if(provider != providers.end()){
        logger::log(logger::LLOG::ERROR, __func__, " Provider with such name is present already. Name: " + name);
    ...
    if(type == "FAKE"){
        ...
        if(!is_real_world()){
            logger::log(logger::LLOG::ERROR, __func__, " GPIO provider MCP23017 can be used with real hardware only");
        ...
        if(!is_real_world()){
            logger::log(logger::LLOG::ERROR, __func__, " GPIO provider PCA9685 can be used with real hardware only");
        ...
        if(!is_real_world()){
            logger::log(logger::LLOG::ERROR, __func__, " SPI bus provider can be used with real hardware only");
    ...
    */
    void PiRobot::add_gpio(const std::string& provider_name, 
        ...
        if(provider->get_ptype() != provider::PROVIDER_TYPE::PROV_GPIO){
            logger::log(logger::LLOG::ERROR, __func__, " Invalid provider type");
